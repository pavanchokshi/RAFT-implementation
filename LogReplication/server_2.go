...
func TestFunc(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	fmt.Println("in server2")
	//q := Query{}
	ok := acknowledgeQuery{}
	ok.acknowledgeQuery = "OK from server 2"
...
func ConnectToMongo() {   //CONVERT INTO BOOLEAN
	...
	//fmt.Println("A")
	fmt.Println("I have to store the string given below")
   ...
   if uri == "" {
        ...
        //fmt.Println("before collection")
      	...
      	collection := sess.DB("raft").C("raftCollection2")
...
func ReceiveCommit(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	fmt.Println("in server2 while commiting")
	Q := CommitQuery{}
	decoder := json.NewDecoder(r.Body)
	err := decoder.Decode(&Q)
	if err != nil {
