...
func initialize() {
	...
	myDetails.Port = "3000"    //enter the port number
...
func NotifyFollowers() {
	...
	for _, val := range nodeList {
		url := fmt.Sprintf("http://%s:%s/iamleader/3000", val.IP, val.Port)
		client := http.Client{}
...
func AskForVote() {
	...
	for _, value := range nodeList {
		if !voteGiven {
			url := fmt.Sprintf("http://localhost:%s/voteforme/3000", value.Port)
			fmt.Println("URL:", url)
			_, err := http.Get(url)
			if err != nil {
				fmt.Println(err)
